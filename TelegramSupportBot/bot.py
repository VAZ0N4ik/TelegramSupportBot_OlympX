...
def send_text(message):
    ...
    if message.text == '✏️ Написать запрос':
        take_new_request = bot.send_message(message.chat.id, 'Просьба отправлять запросы в следующем формате:\n\n*ФИО*\n*Тематика вопроса:* трудности с заданиями или технические проблемы с сайтом\n*Подробное описание вопроса*\n\nМы вернёмся с ответом буквально через пару минут!', reply_markup=markup.markup_cancel(), parse_mode="Markdown")
    ...
    if additional_message == None:
        ...
        if status == 'agent':
            ...
            try:
                ...
                bot.send_message(user_id, f'⚠️ Получен новый ответ на ваш запрос ID {req_id}!\n\n🧑‍💻 Ответ агента поддержки:\n{additional_message}\n\nЧтобы *__продолжить диалог__* по текущему запросу или *__завершить его__*, выберите :\nМои запросы —> ❗️| ID: {req_id}', reply_markup=markup.markup_main(), parse_mode="MarkdownV2")
